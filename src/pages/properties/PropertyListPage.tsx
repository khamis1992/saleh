...
import { Search, Filter, Eye, Pencil, Trash2, Plus, Building2, X, DoorOpen, TrendingUp, Percent } from 'lucide-react';
...
export default function PropertyListPage() {
  ...
  const properties = useMemo(() => {
    const data = propertyStore.getAll();
    ...
    <div className="min-h-full bg-[#f6f9fc]" dir="rtl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KpiCard
          ...
          subtitle={`${filtered.length} ${filtered.length === 1 ? 'عقار' : 'عقار'}`}
          icon={Building2}
          moduleOverride="leasing"
        />
        <KpiCard
          title="إجمالي الوحدات"
          value={totalUnits}
          subtitle={`${leasedUnits} مؤجرة، ${availableUnits} متاحة`}
          icon={DoorOpen}
          moduleOverride="leasing"
        />
        <KpiCard
          title="نسبة الإشغال"
          value={`${occupiedRate}%`}
          subtitle="من إجمالي الوحدات"
          icon={Percent}
          trend={occupiedRate >= 50 ? { value: occupiedRate, label: 'جيد' } : undefined}
          moduleOverride="leasing"
        />
        <KpiCard
          title="قيمة الأصول"
          value={formatQARInt(totalAssetValue)}
          subtitle="إجمالي القيمة الدفترية"
          icon={TrendingUp}
          moduleOverride="leasing"
        />
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {filtered.length} {filtered.length === 1 ? 'عقار' : 'عقار'} — {t.properties.list}
          </p>
        </div>
        <Button
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="overflow-auto">
            <Table>
              <TableHeader>
                <TableRow >
                  <TableHead className="text-[11px] font-bold text-[#64748B] h-9">{t.properties.code}</TableHead>
                  <TableHead className="text-[11px] font-bold text-[#64748B] h-9">{t.properties.name}</TableHead>
                  <TableHead className="text-[11px] font-bold text-[#64748B] h-9">{t.properties.type}</TableHead>
                  <TableHead className="text-[11px] font-bold text-[#64748B] h-9">{t.properties.address}</TableHead>
                  <TableHead className="text-[11px] font-bold text-[#64748B] h-9">{t.properties.assetValue}</TableHead>
                  <TableHead className="text-[11px] font-bold text-[#64748B] h-9">{t.properties.status}</TableHead>
              ...
              <TableBody>
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-16">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm font-medium text-gray-500">لا توجد عقارات</p>
                ...
                {filtered.map((p: any) => (
                  ...
                  >
                    <TableCell className="font-medium text-sm">
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            onClick={(e) => { e.stopPropagation(); handleCopyCode(p.property_code); }}
                    ...
                    <TableCell className="text-sm font-medium text-gray-800">{p.property_name}</TableCell>
                    <TableCell className="text-sm text-gray-600">{(t.properties.types as any)[p.property_type] || p.property_type}</TableCell>
                    <TableCell className="text-sm text-gray-600">{p.address}</TableCell>
                    <TableCell className="text-sm font-mono text-gray-700">{fmt(p.total_asset_value)}</TableCell>
                    <TableCell><StatusBadge status={p.status} label={(t.properties.statuses as any)[p.status]} /></TableCell>
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center gap-1">
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-gray-400 hover:text-blue-600 hover:bg-blue-50" onClick={() => navigate(`/properties/${p.id}`)}>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-gray-400 hover:text-amber-600 hover:bg-amber-50" onClick={() => navigate(`/properties/${p.id}/edit`)}>
          ...
          {filtered.length > 0 && (
            <div className="py-3 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between">
              <span className="text-xs text-gray-500">عرض {filtered.length} من {properties.length} عقار</span>
            </div>
          )}
        </div>
      )}
        ...
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              هل أنت متأكد من حذف العقار <strong>{deleteTarget?.property_name}</strong> ({deleteTarget?.property_code})؟ لا يمكن التراجع عن هذا الإجراء.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
