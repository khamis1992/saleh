    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-[#64748d] mt-0.5">مستندات العقارات، العقود، والصكوك</p>
        </div>
        <Button variant="outline" onClick={() => toast.success(`جاري تحضير ${filtered.length} مستند`)} className="h-10 text-xs">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-1 overflow-x-auto">
          <button onClick={() => setTypeFilter('all')} className={`px-3 h-10 rounded-lg text-xs font-medium whitespace-nowrap ${typeFilter === 'all' ? 'bg-emerald-600 text-white' : 'bg-white text-gray-700 border border-[#e5edf5]'}`}>الكل</button>
          {DOC_TYPES.map((t) => {
            const Icon = t.icon;
            return (
              <button key={t.value} onClick={() => setTypeFilter(t.value)} className={`px-3 h-10 rounded-lg text-xs font-medium whitespace-nowrap flex items-center gap-1.5 ${typeFilter === t.value ? `bg-${t.color}-600 text-white` : 'bg-white text-gray-700 border border-[#e5edf5]'}`}>
            ...
            return (
              <Card key={doc.id} className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px] hover:shadow-[rgba(50,50,93,0.25)_0px_13px_27px_-5px] transition-shadow">
                <CardContent className="p-4">
                  ...
                  {doc.notes && <p className="text-xs text-[#64748d] mb-3 line-clamp-2">{doc.notes}</p>}
                  ...
                  <Button variant="outline" size="sm" className="w-full h-8 text-xs" onClick={() => toast.success(`جاري تحميل ${doc.name}`)}>
