    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
        <div>
          ...
          <p className="text-[12px] text-[#64748d] mt-0.5">بياناتك الشخصية والتواصل</p>
        </div>
        {!editing ? (
          <Button onClick={() => setEditing(true)} className="h-10 text-[12px] bg-[#533afd] hover:bg-blue-700">
            <Edit className="h-4 w-4 ml-1" />
            تعديل البيانات
          </Button>
        ) : (
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => { setEditing(false); setForm(tenant); }} className="h-10 text-[12px]">
              إلغاء
            </Button>
            <Button onClick={handleSave} className="h-10 text-[12px] bg-[#533afd] hover:bg-blue-700">
        ...
        <CardContent className="p-6">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-[12px] text-[#64748d] mt-0.5 flex items-center gap-2">
                <Hash className="h-3 w-3" />
                {tenant.tenant_code}
                {tenant.tenant_type === 'company' && <span className="px-1.5 py-0.5 bg-[rgba(83,58,253,0.06)] text-[#533afd] text-[12px] rounded">{tt('tenants.types.company', 'شركة')}</span>}
                {tenant.tenant_type === 'government' && <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-700 text-[12px] rounded">{tt('tenants.types.government', 'جهة حكومية')}</span>}
                {tenant.tenant_type === 'individual' && <span className="px-1.5 py-0.5 bg-violet-50 text-violet-700 text-[12px] rounded">{tt('tenants.types.individual', 'فرد')}</span>}
              </p>
            </div>
      ...
      <Card className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px]">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <Label className="text-[12px]">{tt('hr.fullName', 'الاسم الكامل')}</Label>
            {editing ? (
          ...
          {tenant.tenant_type === 'company' && (
            <div>
              <Label className="text-[12px]">اسم الشركة</Label>
              {editing ? (
          ...
          <div>
            <Label className="text-[12px]">رقم الهوية / الجواز</Label>
            {editing ? (
          ...
          <div>
            <Label className="text-[12px]">{tt('hr.nationality', 'الجنسية')}</Label>
            {editing ? (
          ...
          {tenant.tenant_type !== 'individual' && (
            <>
              <div>
                <Label className="text-[12px]">جهة العمل</Label>
                {editing ? (
              ...
              <div>
                <Label className="text-[12px]">المخول بالتوقيع</Label>
                {editing ? (
      ...
      <Card className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px]">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <Label className="text-[12px]">{tt('hr.phone', 'رقم الجوال')}</Label>
            {editing ? (
          ...
          <div className="md:col-span-2">
            <Label className="text-[12px]">{tt('properties.address', 'العنوان')}</Label>
            {editing ? (
      ...
      <Card className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px]">
        <CardHeader>
          ...
          <CardDescription className="text-[12px]">سيتم الاتصال بهم في حالة الطوارئ</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <Label className="text-[12px]">{tt('tenants.name', 'الاسم')}</Label>
            {editing ? (
      ...
      <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2">
        ...
        <p className="text-[12px] text-[#9b6829]">
