    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-[#64748d] mt-0.5">العقود النشطة والمنتهية</p>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2">
            <p className="text-xs text-emerald-600">قيمة العقود النشطة</p>
            <p className="text-sm font-bold text-emerald-700">{fmtInt(totalActive)}</p>
          </div>
          <div className="bg-[rgba(83,58,253,0.06)] border border-blue-200 rounded-xl px-3 py-2">
            <p className="text-xs text-[#533afd]">إجمالي الدفعات المقدمة</p>
            <p className="text-sm font-bold text-[#533afd]">{fmtInt(totalAdvance)}</p>
          </div>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <Card className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px]">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-[#64748d] text-sm">لا توجد عقود</p>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <h3 className="text-sm font-bold text-[#061b31]">{c.contract_title}</h3>
                        <StatusBadge status={c.status} />
                      </div>
                      <p className="text-xs text-[#64748d] flex items-center gap-2 mb-3">
                      ...
                      {c.status === 'active' && (
                        <div className="mt-3">
                          <div className="flex items-center justify-between mb-1">
                            <p className="text-xs text-[#64748d]">التقدم الزمني</p>
                            <p className="text-xs text-[#64748d]">{progress}% · {daysToEnd > 0 ? `${daysToEnd} يوم متبقي` : 'منتهي'}</p>
                          </div>
                    ...
                    <div className="flex flex-col gap-2 lg:min-w-[140px]">
                      ...
                      <Button variant="outline" size="sm" className="w-full h-9 text-xs" onClick={() => toast.success(`جاري تحميل ${c.contract_number}`)}>
