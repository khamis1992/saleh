  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-[#64748d] mt-0.5">تتبع مدفوعات مستحقاتك من المطالبات</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px]">
          <CardContent className="p-5">
            <p className="text-xs text-[#64748d]">إجمالي المطالبات</p>
            <p className="text-xl font-bold text-[#061b31]">{fmt(totalClaimed)}</p>
            <p className="text-xs text-[#64748d] mt-1">{myClaims.length} مطالبة</p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px]">
          <CardContent className="p-5">
            ...
            <p className="text-xs text-emerald-600 mt-1">تم استلامه</p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px]">
          <CardContent className="p-5">
            <p className="text-xs text-[#533afd]">معتمد بانتظار الدفع</p>
            <p className="text-xl font-bold text-[#533afd]">{fmt(totalPending)}</p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px]">
          <CardContent className="p-5">
            <p className="text-xs text-[#9b6829]">إجمالي معتمد</p>
      ...
      <Card className="border-0 shadow-[rgba(50,50,93,0.20)_0px_4px_8px_-2px]">
        <CardContent className="p-0">
          <div className="p-4 border-b border-[#e5edf5]">
            <h2 className="text-sm font-bold text-[#061b31]">سجل المدفوعات</h2>
            <p className="text-xs text-[#64748d]">آخر المطالبات وحالة سدادها</p>
          </div>
          {sorted.length === 0 ? (
                  ...
                  <div key={c.id} className="p-4 hover:bg-[#f6f9fc] transition-colors">
                    <div className="flex items-center justify-between gap-3 flex-wrap">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium bg-${ps.color}-50 text-${ps.color}-700`}>
                              {ps.label}
                            </span>
                          </div>
                      ...
                      <div className="text-left flex items-center gap-3">
                        <div>
                          <p className="text-xs text-[#64748d]">صافي</p>
                          <p className="text-sm font-bold text-[#061b31]">{fmt(c.net_payable)}</p>
                        </div>
                        {c.payment_status !== 'paid' && (
                          <Button size="sm" variant="outline" className="h-8 text-xs" onClick={() => toast.info('تذكير بالإدارة')}>
                            تذكير
                          </Button>
                        )}
                      </div>
