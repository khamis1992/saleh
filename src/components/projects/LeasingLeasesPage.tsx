...
import {
  Search, Filter, Eye, Pencil, Trash2, Plus, Play, FileText, X, CreditCard,
  AlertCircle, TrendingUp, TrendingDown, RotateCcw, Sparkles, Users, Award,
  MapPin, CalendarDays, DollarSign, Activity, ArrowRight, Clock, AlertTriangle,
} from 'lucide-react';
...
const leaseStatusConfig: Record<string, { dot: string; chip: string }> = {
  ...
  expiring_soon:   { dot: 'bg-amber-500', chip: 'bg-amber-50 text-amber-700 ring-1 ring-amber-100' },
  terminated:      { dot: 'bg-rose-500', chip: 'bg-rose-50 text-rose-700 ring-1 ring-rose-100' },
};
...
function LeaseCard({ l, units, properties, onDelete, onActivate }: {
  l: any; units: any[]; properties: any[]; onDelete: (l: any) => void; onActivate: (l: any) => void;
}) {
  const navigate = useNavigate();
  const { t } = useLocale();
  const statCfg = leaseStatusConfig[l.status] || leaseStatusConfig.draft;
  const unit = units.find((u: any) => u.id === l.unit_id);
  const prop = properties.find((p: any) => p.id === (l.property_id || unit?.property_id));
  const tenantName = l.tenant_name || l.tenant_id || '';
  const isExpiring = l.status === 'active' && l.end_date && new Date(l.end_date) < new Date(Date.now() + 30 * 86400000);
  ...
  return (
    <div onClick={() => navigate(`/leases/${l.id}`)}
      ...
      <div className={`absolute top-0 left-0 right-0 h-[3px] rounded-t-xl opacity-60 ${isExpiring ? 'bg-amber-500' : l.status === 'active' ? 'bg-emerald-500' : l.status === 'draft' ? 'bg-gray-300' : 'bg-rose-500'}`} />

      <div className="flex items-start justify-between mb-3.5">
        <div className="flex items-center gap-3 min-w-0">
          <div className="h-10 w-10 rounded-lg bg-rose-50 text-rose-600 ring-1 ring-rose-100 flex items-center justify-center shrink-0">
            <FileText className="h-5 w-5" />
          </div>
          <div className="min-w-0">
            <div className="text-sm font-bold text-gray-900 truncate">{l.contract_number}</div>
            <div className="text-[11px] text-gray-500 mt-0.5 truncate">{tenantName}</div>
          </div>
        </div>
        <span className={`inline-flex items-center gap-1.5 h-7 px-2.5 rounded-md text-[11px] font-bold ${statCfg.chip}`}>
          ...
          {(t.leases.statuses as any)[l.status] || l.status}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-[11px] text-gray-500 mb-3.5">
        ...
        <span className="flex items-center gap-1.5"><Activity className="h-3 w-3 text-gray-400" />وحدة {unit?.unit_number || l.unit_id || '—'}</span>
      </div>

      <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-[11px] text-gray-500 mb-3.5">
        <span className="flex items-center gap-1.5"><CalendarDays className="h-3 w-3 text-gray-400" />{l.start_date} → {l.end_date}</span>
      </div>

      <div className="bg-gray-50 rounded-lg p-3 mb-3.5">
        <div className="text-[10px] text-gray-400 mb-0.5">الإيجار الشهري</div>
        <div className="text-lg font-bold text-gray-900 ltr-only tabular-nums">{fmt(l.rent_amount)}</div>
        <div className="text-[10px] text-gray-400 mt-0.5">{(t.leases.frequencies as any)[l.payment_frequency] || l.payment_frequency}</div>
      </div>

      <div className="flex items-center gap-1 pt-2 border-t border-gray-50" onClick={e => e.stopPropagation()}>
        <button onClick={() => navigate(`/leases/${l.id}`)} className="h-7 w-7 rounded-md text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors flex items-center justify-center">
          <Eye className="h-3.5 w-3.5" />
        </button>
        <button onClick={() => navigate(`/leases/${l.id}/edit`)} className="h-7 w-7 rounded-md text-gray-400 hover:text-amber-600 hover:bg-amber-50 transition-colors flex items-center justify-center">
          <Pencil className="h-3.5 w-3.5" />
        </button>
        {l.status === 'draft' && (
          <button onClick={() => onActivate(l)} className="h-7 px-2 rounded-md text-[10px] font-bold text-emerald-600 hover:bg-emerald-50 transition-colors flex items-center gap-1">
            <Play className="h-3 w-3" /> تفعيل
          </button>
        )}
        <button onClick={() => onDelete(l)} className="h-7 w-7 rounded-md text-gray-400 hover:text-rose-600 hover:bg-rose-50 transition-colors flex items-center justify-center">
          <Trash2 className="h-3.5 w-3.5" />
        </button>
        <div className="flex-1" />
        <button onClick={() => navigate(`/leases/${l.id}`)} className="flex items-center gap-1 text-[11px] font-bold text-gray-500 hover:text-rose-600 transition-colors">
...
function LeaseRow({ l, units, properties, onDelete, onActivate }: {
  l: any; units: any[]; properties: any[]; onDelete: (l: any) => void; onActivate: (l: any) => void;
}) {
  const navigate = useNavigate();
  const { t } = useLocale();
  ...
  return (
    <tr className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors cursor-pointer" onClick={() => navigate(`/leases/${l.id}`)}>
      <td className="px-4 py-3">
        <Tooltip><TooltipTrigger asChild>
          <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(l.contract_number).then(() => toast.success('تم نسخ رقم العقد')); }}
            className="font-mono text-xs text-rose-600 hover:text-rose-700 transition-colors">{l.contract_number}</button>
        </TooltipTrigger><TooltipContent>اضغط للنسخ</TooltipContent></Tooltip>
      </td>
      <td className="px-4 py-3"><span className="text-sm font-bold text-gray-900">{tenantName}</span></td>
      <td className="px-4 py-3 text-xs text-gray-600">{unit?.unit_number || l.unit_id || '—'}</td>
      <td className="px-4 py-3 text-xs text-gray-600 max-w-[160px] truncate">{prop?.property_name || '—'}</td>
      <td className="px-4 py-3 text-xs text-gray-600">{l.start_date}</td>
      <td className="px-4 py-3 text-xs text-gray-600">{l.end_date}</td>
      <td className="px-4 py-3 text-xs font-mono text-gray-700 ltr-only tabular-nums">{fmt(l.rent_amount)}</td>
      <td className="px-4 py-3 text-xs text-gray-600">{(t.leases.frequencies as any)[l.payment_frequency] || l.payment_frequency}</td>
      <td className="px-4 py-3">
        <span className={`inline-flex items-center gap-1.5 h-7 px-2.5 rounded-md text-[11px] font-bold ${statCfg.chip}`}>
          <span className={`h-1.5 w-1.5 rounded-full ${statCfg.dot}`} />
          {(t.leases.statuses as any)[l.status] || l.status}
      ...
      <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-1">
          <Tooltip><TooltipTrigger asChild><button onClick={() => navigate(`/leases/${l.id}`)} className="h-7 w-7 rounded-md text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors flex items-center justify-center"><Eye className="h-3.5 w-3.5" /></button></TooltipTrigger><TooltipContent>عرض</TooltipContent></Tooltip>
          <Tooltip><TooltipTrigger asChild><button onClick={() => navigate(`/leases/${l.id}/edit`)} className="h-7 w-7 rounded-md text-gray-400 hover:text-amber-600 hover:bg-amber-50 transition-colors flex items-center justify-center"><Pencil className="h-3.5 w-3.5" /></button></TooltipTrigger><TooltipContent>تعديل</TooltipContent></Tooltip>
          {l.status === 'draft' && (
            <Tooltip><TooltipTrigger asChild><button onClick={() => onActivate(l)} className="h-7 w-7 rounded-md text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors flex items-center justify-center"><Play className="h-3.5 w-3.5" /></button></TooltipTrigger><TooltipContent>تفعيل العقد</TooltipContent></Tooltip>
          )}
          <Tooltip><TooltipTrigger asChild><button onClick={() => onDelete(l)} className="h-7 w-7 rounded-md text-gray-400 hover:text-rose-600 hover:bg-rose-50 transition-colors flex items-center justify-center"><Trash2 className="h-3.5 w-3.5" /></button></TooltipTrigger><TooltipContent>حذف</TooltipContent></Tooltip>
        </div>
      </td>
    </tr>
  );
...
function EmptyLeases({ onReset }: { onReset: () => void }) {
  return (
    <div className="flex flex-col items-center gap-4 py-16 bg-white rounded-xl border border-gray-100">
      ...
      <div className="text-center">
        <p className="text-sm font-bold text-gray-700">لا توجد عقود</p>
...
export default function LeasingLeasesPage() {
  ...
  const filtered = leases.filter((l: any) => {
    ...
    const tenantName = l.tenant_name || l.tenant_id || '';
    if (search && !tenantName.includes(search) && !l.contract_number.includes(search)) return false;
    return true;
  });
  ...
  const totalRentValue = leases.reduce((s: number, l: any) => s + (l.rent_amount || 0), 0);
  ...
  function handleActivate(lease: any) {
    ...
    if (result) {
      ...
      setRefresh(r => r + 1);
    } else toast.error('فشل تفعيل العقد');
  }
    ...
    <div className="min-h-full bg-[#fafbfc]" dir={dir}>
        ...
        <div className="max-w-[1640px] mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center gap-3">
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="hidden md:flex flex-col leading-tight">
              <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-rose-600">عقود الإيجار</span>
              <span className="text-[13px] font-bold text-gray-900">{leases.length} عقد</span>
            </div>
          </div>
          <div className="h-6 w-px bg-gray-200 hidden md:block" />
          <div className="relative flex-1 max-w-xs">
            <Search className="absolute end-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-400 pointer-events-none" />
            <Input placeholder="ابحث في العقود..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <div className="hidden lg:flex items-center gap-2 text-[11px] text-gray-500">
            <span>قيمة الإيجارات:</span>
            <span className="font-bold text-gray-900 ltr-only tabular-nums">{fmtInt(totalRentValue)}</span>
          </div>
          <div className="me-auto" />
          <div className="flex items-center bg-gray-100/80 rounded-lg p-0.5" role="tablist">
            {([
              { key: 'list', label: 'قائمة', icon: Sparkles },
              { key: 'grid', label: 'بطاقات', icon: FileText },
            ] as const).map(v => (
          ...
          <Button onClick={() => navigate('/leases/create')}
            className="h-8 px-3 gap-1.5 bg-rose-500 hover:bg-rose-600 text-white text-[11px] font-bold rounded-lg shadow-sm">
            <Plus className="h-3.5 w-3.5" /><span>إضافة عقد</span>
          </Button>
        </div>
      </div>
      ...
      <div className="max-w-[1640px] mx-auto px-4 sm:px-6 lg:px-8 py-5 space-y-5">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <KpiCard label="إجمالي العقود" value={leases.length} sub={`${activeContracts} عقد نشط`} icon={FileText} accent="slate" />
          <KpiCard label="تنتهي قريباً" value={expiringSoon} sub="خلال 30 يوم" icon={AlertCircle} trend={{ val: expiringSoon, dir: 'down' }} accent="amber" />
          <KpiCard label="متأخرات" value={overdueInvoices} sub="فواتير متأخرة" icon={AlertTriangle} trend={{ val: overdueInvoices, dir: 'down' }} accent="rose" />
          <KpiCard label="قيمة الإيجارات" value={fmtInt(totalRentValue)} sub="الإيجار الشهري الإجمالي" icon={CreditCard} accent="emerald" />
        </div>

        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-bold text-gray-900">{t.leases.title}</h2>
        ...
        {filtered.length === 0 ? (
          <EmptyLeases onReset={resetFilters} />
        ) : viewMode === 'grid' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
            {filtered.map(l => <LeaseCard key={l.id} l={l} units={units} properties={properties} onDelete={setDeleteTarget} onActivate={handleActivate} />)}
          </div>
        ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/40">
                    <th className="text-[11px] font-bold text-gray-500 h-9 px-4 text-right">رقم العقد</th>
                    ...
                    <th className="text-[11px] font-bold text-gray-500 h-9 px-4 text-right">تاريخ البداية</th>
                    <th className="text-[11px] font-bold text-gray-500 h-9 px-4 text-right">تاريخ النهاية</th>
                    <th className="text-[11px] font-bold text-gray-500 h-9 px-4 text-right">الإيجار</th>
                    <th className="text-[11px] font-bold text-gray-500 h-9 px-4 text-right">الدورية</th>
                ...
                <tbody>
                  {filtered.map(l => <LeaseRow key={l.id} l={l} units={units} properties={properties} onDelete={setDeleteTarget} onActivate={handleActivate} />)}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between text-[11px] text-gray-500 px-1">
          <span>عرض {filtered.length} من {leases.length} عقد</span>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setDeleteTarget(null)}>
          <div className="bg-white rounded-xl border border-gray-100 shadow-xl p-6 w-full max-w-sm" dir={dir} onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-600 mb-5">هل أنت متأكد من حذف العقد <strong className="text-gray-900">{deleteTarget.contract_number}</strong>؟</p>
